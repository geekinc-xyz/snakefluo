...
export function GameScene() {
  ...
  useFrame((state, delta) => {
    ...
    if (serverPlayer && serverPlayer.state === 'alive') {
      ...
      const rotationStep = TURN_SPEED * delta;
      if (Math.abs(angleDiff) < rotationStep) {
        localPlayerRef.current.currentAngle = targetAngle;
      } else {
        localPlayerRef.current.currentAngle += Math.sign(angleDiff) * rotationStep;
      }

      // Keyboard fallback (optional, but we'll prioritize mouse if it's moving)
